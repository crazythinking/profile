...
(
	...
	DEL_FLG BOOLEAN COMMENT '逻辑删除标识',
...
(
	...
	-- 菜单代码,不同app下可以相同
	MENU_CD VARCHAR(30) NOT NULL COMMENT '菜单代码 : 菜单代码,不同app下可以相同',
...
(
	...
	-- 接入授权中心对应的client_id
	APP_CD VARCHAR(50) COMMENT '应用代码 : 接入授权中心对应的client_id',
	-- 作为Auth的权限标识，以及前端的接口标识，不同app下可以相同
	INTERF_CD VARCHAR(50) NOT NULL COMMENT '接口代码 : 作为Auth的权限标识，以及前端的接口标识，不同app下可以相同',
	INAME VARCHAR(100) NOT NULL COMMENT '接口名称',
	-- 接口代码所属菜单ID
	MENU_ID INT NOT NULL COMMENT '菜单ID : 接口代码所属菜单ID',
	MTN_TIMESTAMP TIMESTAMP COMMENT '修改时间',
	MTN_USER VARCHAR(40) COMMENT '修改用户',
	JPA_VERSION INT NOT NULL COMMENT '乐观锁版本号',
...
(
	...
	-- 当权限对应页面从属接口时，使用此字段对应菜单ID；当权限对应独立接口或菜单时，此字段为‘-’或URI
	AUTU_URI VARCHAR(500) NOT NULL COMMENT '权限URI : 当权限对应页面从属接口时，使用此字段对应菜单ID；当权限对应独立接口或菜单时，此字段为‘-’或URI',
	CREATE_USER VARCHAR(40) COMMENT '创建用户',
	CREATE_TIMESTAMP TIMESTAMP COMMENT '创建时间',
	JPA_VERSION INT DEFAULT 0 NOT NULL COMMENT '乐观锁版本号',
