...
//    @PreAuthorize("hasAuthority('Maintenance')")
	...
	CommonWithHeaderResponse getUserByName(@RequestBody ProfileUserForm user, HttpServletRequest request) {
		...
		profileSecurityLoggerService.logSecuOperation(puid.toString(), OperationType.QU, WebCommonUtils.getIpAddress(request),date,user.getUserId());
	...
	CommonWithHeaderResponse loginUserByName(@RequestBody ProfileUserForm user, HttpServletRequest request) {
		...
		Object puid =profileSecurityLoggerService.getPuid(user.getOperUserId());
		//userNo是操作员的puid，beOper是被操作员的登录id
		profileSecurityLoggerService.logSecuOperation(puid.toString(), OperationType.LG, WebCommonUtils.getIpAddress(request),date,user.getUserId());
...
//    @PreAuthorize("hasAuthority('PuId')")
	...
	CommonWithHeaderResponse addUser(@RequestBody MgmWebUser user, HttpServletRequest request) {
		...
		Object puid =profileSecurityLoggerService.getPuid(user.getOperUserId());
		//userNo是操作员的puid，beOper是被操作员的登录id
		profileSecurityLoggerService.logSecuOperation(puid.toString(), OperationType.AD, WebCommonUtils.getIpAddress(request),date,user.getUserId());
	...
	CommonWithHeaderResponse removeUser(@RequestBody ProfileUserForm user, HttpServletRequest request) {
		...
		Object puid =profileSecurityLoggerService.getPuid(user.getOperUserId());
		//userNo是操作员的puid，beOper是被操作员的登录id
		profileSecurityLoggerService.logSecuOperation(puid.toString(), OperationType.DE, WebCommonUtils.getIpAddress(request),date,userId);
		return new CommonWithHeaderResponseBuilder<Void,Void>().build();
	}
...
//    @PreAuthorize("hasAuthority('Maintenance')")
	...
	CommonWithHeaderResponse updateUser(@RequestBody ProfileUserForm user, HttpServletRequest request) {
		...
		Object puid =profileSecurityLoggerService.getPuid(user.getOperUserId());
		//userNo是操作员的puid，beOper是被操作员的登录id
		profileSecurityLoggerService.logSecuOperation(puid.toString(), OperationType.UP, WebCommonUtils.getIpAddress(request),date,user.getUserId());
		return new CommonWithHeaderResponseBuilder<Void,Void>().build();
	}
